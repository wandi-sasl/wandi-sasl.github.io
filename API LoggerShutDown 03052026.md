<a id="_Hlk91953239"></a><a id="_Hlk91953555"></a>__Use LoggerShutDown\(\) in an application__
...
Frees the client internal data structure and terminates logger services\.

LoggerShutDown\(\) returns 0 on success and \-1 on error\. <a id="_Hlk221803928"></a>When an error occurs additional error codes are written to the following files \(where <cpid> is the client process id number and <spid>\. Is the sever process id number\):
